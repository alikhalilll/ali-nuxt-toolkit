...
| `trimDisplay`        | `boolean`                                        | `false`                      | When a country is already pinned (e.g. via `defaultCountry`) and the user types a local format, this controls whether the visible input is also rewritten to the trunk-zero-stripped national number once typing settles. Off by default — display stays as typed, only the bound model normalizes. |
| `ipEndpoint`         | `string`                                         | `'https://ipapi.co/json/'`   | Override the IP geolocation endpoint.                                                                                                                                                                                                                                                               |
| `forceBottomSheet`   | `boolean`                                        | `false`                      | Force the country picker to render as a bottom-sheet drawer regardless of viewport width — desktops get the vaul drawer too. Useful for primarily-mobile flows where the picker should feel modal on every form factor.                                                                             |
...
- **Trunk-zero stripping.** With a country pinned (e.g. `default-country="20"`), typing `01066105963` binds `v-model:phone` to the canonical `1066105963` after the settle window — the leading trunk zero is dropped so the model matches libphonenumber's `nationalNumber`, and the composed E.164 v-model is `+201066105963`. The visible input stays as typed by default. Pass `trim-display` if you want the field to also snap to the stripped form once the user pauses.
