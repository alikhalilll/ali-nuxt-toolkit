...
| [`@alikhalilll/a-*`](/ui)                               | Vue 3 library | Headless components — `ATelInput`, `APopover`, `ADrawer`, …. One package per component. |
...
`@alikhalilll/a-*` is **not** a Nuxt module — it's a Vue 3 component library that works in any Vue 3 app. See [its setup guide](/ui#setup) for the one-time CSS + Tailwind tokens wiring.
...
- **Pick what you need.** Install one module, or all three, or just the UI lib. Inside `@alikhalilll/a-*`, each component is its own package (`@alikhalilll/a-popover`, …) so consumers only pay for what they touch.
